...
def main(opt):
    ...
    def on_end_epoch(hook_state, state):
        ...
        # val_acc = meter_vals['val']['acc']
        # num = np.array([[train_acc,val_acc]])
        # data_temp = pd.DataFrame(data=num, columns=data_log.columns)
        # data_log = data_log.append(data_temp)
        # data_log.to_csv(file_path, index=None)
