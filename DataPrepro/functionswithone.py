...
def csv_joint(num_li):
    ...
    for i in num_li:
        name="D:/Users/oyyk/PycharmProjects/F_G_P/data/RawData/groups/Muticlass_csv_"+str(i)+".csv"
...
#单个文件按分类重新打上标签
def label_set(data,scenes):
    for i in range(data.shape[0]):
        label=data['label'][i]
        print(i,end=" ")
        if label in scenes[0]:
            print("detected in 0")
            data.loc[i, 'label'] = str(0)
        elif label in scenes[1]:
            print("detected in 1")
            data.loc[i, 'label'] = str(1)
        elif label in scenes[2]:
            print("detected in 2")
            data.loc[i, 'label'] = str(2)
        elif label in scenes[3]:
            print("detected in 3")
            data.loc[i, 'label'] = str(3)
        elif label in scenes[4]:
            print("detected in 4")
            data.loc[i, 'label'] = str(4)
    return data

def cut_frames(frames,num=40):
    for i in range(len(frames)):
        group_name="D:/Users/oyyk/PycharmProjects/F_G_P/data/RawData/groups/group"+str(i)+"/"
        results_name="D:/Users/oyyk/PycharmProjects/F_G_P/data/RawData/groups/"
        temp_data=frames[i]
        gap=temp_data.shape[0]//num
        print(gap)
        temp_name = results_name + str(i) + ".csv"
